...
window.addEventListener('load', () => {
    // Update image info values on initial load
    imageDensity.textContent = informationDensity.value;
    imageKnowledge.textContent = knowledgeBase.value;
    imageComplexity.textContent = cognitiveComplexity.value;
...
function createInfoPanel() {
    if (infoPanel) return; // Prevent duplicate panels
    ...
    `;
    document.body.appendChild(infoPanel); // Use body to avoid container overflow
    // Add close button functionality
    infoPanel.querySelector('.close-btn').addEventListener('click', () => {
        infoPanel.style.display = 'none';
        selectedElement = null;
    });
    // Always ensure panel is visible
    infoPanel.style.display = 'block';
    infoPanel.style.zIndex = 99999;
    infoPanel.style.position = 'absolute';
    infoPanel.style.pointerEvents = 'auto';
    infoPanel.style.background = '#fffbe6';
    infoPanel.style.border = '2px solid #3498db';
    infoPanel.style.maxWidth = '300px';
    infoPanel.style.boxShadow = '0 4px 24px rgba(0,0,0,0.2)';
}
...
function showInfoPanel(element) {
    ...
    // Debug
    console.log('showInfoPanel called for element:', element);
    ...
    switch (element.type) {
        ...
        case 'connection':
            ...
            break;
        case 'point':
            description = `This point represents a discrete unit of information in your consciousness profile.<br><br>`;
            if (element.angle !== undefined) {
                const angleDegrees = Math.round(element.angle * 180 / Math.PI);
                description += `Position: ${angleDegrees}° from center<br>`;
            }
            if (element.density !== undefined) {
                description += `Information Density: ${element.density}<br>`;
            }
            if (element.knowledge !== undefined) {
                description += `Knowledge Base: ${element.knowledge}<br>`;
            }
            if (element.complexity !== undefined) {
                description += `Cognitive Complexity: ${element.complexity}<br>`;
            }
            description += `<br>Each point on the concentric circles is a symbolic representation of a unique informational element that contributes to your overall consciousness visualization.`;
            break;
        case 'vortex':
    ...
    elementDescription.innerHTML = `Description: ${description}`;
    // Position the info panel near the clicked element
    if (element.x !== undefined && element.y !== undefined) {
        const rect = canvas.getBoundingClientRect();
        const scaleX = rect.width / canvas.width;
        const scaleY = rect.height / canvas.height;
        const panelX = element.x * scaleX + rect.left;
        const panelY = element.y * scaleY + rect.top;
        const windowWidth = window.innerWidth;
        if (panelX + 250 > windowWidth) {
            infoPanel.style.left = `${panelX - 270}px`;
        } else {
            infoPanel.style.left = `${panelX + 20}px`;
        }
        const windowHeight = window.innerHeight;
        if (panelY + 150 > windowHeight) {
            infoPanel.style.top = `${panelY - 170}px`;
        } else {
            infoPanel.style.top = `${panelY + 20}px`;
        }
    } else {
...
canvas.addEventListener('click', (event) => {
    ...
    for (let i = interactiveElements.length - 1; i >= 0; i--) {
        ...
        if (element.type === 'circle' && isPointInCircle(x, y, element.centerX, element.centerY, element.radius)) {
            ...
            showInfoPanel(element);
            return;
        } else if (element.type === 'point' && isPointInCircle(x, y, element.x, element.y, 20)) { // Increased click radius for points
            console.log('Point clicked:', element);
            selectedElement = element;
            showInfoPanel(element);
            return;
...
function updateValueDisplay(input) {
    ...
    // Update corresponding image info value
    switch(input.id) {
        case 'informationDensity':
            imageDensity.textContent = input.value;
            break;
        case 'knowledgeBase':
            imageKnowledge.textContent = input.value;
            break;
        case 'cognitiveComplexity':
            imageComplexity.textContent = input.value;
            break;
    }
}
...
function drawArt(D, A, S) {
    ...
    // Update image info values when art is drawn
    imageDensity.textContent = D;
    imageKnowledge.textContent = A;
    imageComplexity.textContent = S;
    ...
    if (quantumMode.checked) {
        ...
        for (let i = 0; i < D / 5; i++) {
            ...
            // Draw point with larger visual size
            ctx.beginPath();
            ctx.arc(x, y, 8, 0, Math.PI * 2); // Increased visual size
            ctx.fillStyle = `rgba(255, 255, 255, ${0.3 + (A / 100) * 0.7})`;
            ctx.fill();
            
            // Add glow effect for better visibility
            const gradient = ctx.createRadialGradient(x, y, 0, x, y, 12);
            gradient.addColorStop(0, `rgba(255, 255, 255, ${0.3 + (A / 100) * 0.7})`);
            ...
            // Add to interactive elements with more detailed information
            interactiveElements.push({
                type: 'point',
                x: x,
                y: y,
...
function playQuantumToggleSound() {
    try {
...
document.addEventListener('DOMContentLoaded', () => {
    // Quantum mode toggle event listener
    if (quantumMode) {
        quantumMode.addEventListener('change', () => {
            if (quantumMode.checked) {
                playQuantumToggleSound();
...
// Update Save Art button functionality
saveBtn.addEventListener('click', () => {
    // Save the generated image if available, otherwise save the canvas
...
shareBtn.addEventListener('click', async () => {
    // Use the generated image if available, otherwise use the canvas
    const imageDataUrl = currentImage || canvas.toDataURL('image/png');
    const tweetText = "Check out my consciousness art generated with ConsciousCanvas!";
    const projectUrl = window.location.origin;

    // Try Web Share API (modern browsers, especially on mobile)
    if (navigator.canShare && navigator.canShare({ files: [] })) {
        try {
            ...
            // If sharing fails, fall back to Twitter
        }
    }
    // Fallback: Twitter intent
    const tweetUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(tweetText)}&url=${encodeURIComponent(projectUrl)}`;
...
// Update image preview visibility handling
...
// Remove image preview event listeners but keep other functionality
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (generateAIBtn) {
        generateAIBtn.addEventListener('click', () => {
            ...
            // You can implement AI image generation here
        });
    }
});
